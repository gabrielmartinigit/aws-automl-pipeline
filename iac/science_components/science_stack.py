...
class ScienceStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        default_vpc_id = ec2.Vpc.from_lookup(self, "VPC", is_default=True)

        self.vpc_id = default_vpc_id.vpc_id
        self.public_subnet_ids = [
            public_subnet.subnet_id
            for public_subnet in default_vpc_id.public_subnets
        ]

        my_sagemaker_domain = (
            sagemakerStudioConstructs.SagemakerStudioDomainConstruct(
                self,
